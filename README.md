...
```
src/
├── app/                 # Основная конфигурация приложения и роутинг
├── assets/              # Статические ресурсы (иконки, изображения) Иконки как компоненты, необходимо для Playwright
├── components/          # Основные компоненты приложения
├── pages/               # Страницы 
├── shared/              # Общие утилиты, типы, хуки, константы,сторы, ройвайдеры, контексты - только если используются в нескольких местах
└── styles/              # Глобальные стили и переменные
```

### Роутинг

В проекте используется [React Router](https://reactrouter.com/) v7 для навигации между страницами. Роуты определяются в файле [`src/app/routes.ts`](src/app/routes.ts).

### Сборка проекта

Сборкой проекта занимается React Router, который использует Vite под капотом. Это позволяет использовать все преимущества современной сборки и оптимизации.

### SSR (Server-Side Rendering)

В текущей конфигурации SSR отключен. Приложение работает как SPA (Single Page Application). В будущем планируется внедрение SSR для улучшения SEO и производительности.
