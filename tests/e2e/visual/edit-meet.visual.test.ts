...
/**
 * Visual regression for `/meet/edit/:hash`.
 * Baselines live under `tests/e2e/screenshots/` (see `snapshotPathTemplate` in playwright.config).
 * Only `chromium` desktop project to avoid cross-engine font/layout diffs.
 *
 * Update snapshots after intentional UI changes:
 *   NODE_ENV=test pnpm exec playwright test tests/e2e/visual/edit-meet.visual.test.ts --project=chromium --update-snapshots
 */
...
function baseMeet(overrides: Partial<MeetWire> = {}): MeetWire {
  return {
    name: "E2E — edit screenshots",
    description: "Описание для визуальных тестов редактирования.",
...
test.describe("Edit meet page screenshots", () => {
  test("edit page viewport", async ({ page }, testInfo) => {
    test.skip(
    ...
    await expect(page).toHaveScreenshot("edit-meet-viewport.png", {
      animations: "disabled",
      fullPage: false,
      threshold: 0.2,
    });
  ...
  test("edit form block", async ({ page }, testInfo) => {
    test.skip(
      testInfo.project.name !== "chromium",
      "Visual baselines are maintained only for the chromium desktop project.",
    );
    ...
    await expect(form).toHaveScreenshot("edit-meet-form.png", {
      animations: "disabled",
      threshold: 0.2,
    });
  });
