...
/**
 * Visual regression for `/meet/:hash`.
 ...
 *   NODE_ENV=test pnpm exec playwright test tests/e2e/visual/meet.visual.test.ts --project=chromium --update-snapshots
 */
...
function baseMeet(overrides: Partial<MeetWire> = {}): MeetWire {
  return {
    name: "E2E — meet screenshots",
    description: "Описание для визуальных тестов.",
...
test.describe("Meet page screenshots", () => {
  test("meet page viewport", async ({ page }, testInfo) => {
    test.skip(
    ...
    await expect(page).toHaveScreenshot("meet-viewport.png", {
      animations: "disabled",
      fullPage: false,
      threshold: 0.2,
    });
  ...
  test("meet header + table block", async ({ page }, testInfo) => {
    test.skip(
      testInfo.project.name !== "chromium",
      "Visual baselines are maintained only for the chromium desktop project.",
    );
    ...
    await expect(scope).toHaveScreenshot("meet-page-block.png", {
      animations: "disabled",
      threshold: 0.2,
    });
  });
