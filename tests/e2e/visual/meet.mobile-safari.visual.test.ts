...
/**
 * Visual regression for `/meet/:hash` on iOS (mobile-safari / WebKit).
 *
 * Update snapshots:
 *   NODE_ENV=test pnpm exec playwright test tests/e2e/visual/meet.mobile-safari.visual.test.ts --project=mobile-safari --update-snapshots
 */
...
function baseMeet(overrides: Partial<MeetWire> = {}): MeetWire {
  return {
    name: "E2E — meet iOS screenshots",
    description: "Описание для iOS визуального теста.",
...
test.describe("Meet page iOS screenshots", () => {
  test("meet page viewport (iOS)", async ({ page }, testInfo) => {
    ...
    await expect(page).toHaveScreenshot("meet-ios-viewport.png", {
      animations: "disabled",
      fullPage: false,
      threshold: 0.2,
    });
  ...
  test("meet page block (iOS)", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name !== "mobile-safari", "Baselines are maintained only for mobile-safari.");
    ...
    await expect(scope).toHaveScreenshot("meet-page-ios.png", {
      animations: "disabled",
      threshold: 0.2,
    });
  });
